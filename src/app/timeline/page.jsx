...
const iconConfig = {
  ...
  video: {
    icon: <MdOutlineVideocam size={20} />,
    label: "Video",
...
const Timeline = () => {
  ...
  const filteredData =
    filter === "all"
      ? timelineHistory
      : timelineHistory.filter((item) => item.contact === filter);
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {filteredData.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-gray-400">
          <p className="text-lg">No interactions found.</p>
        </div>
      ) : (
        <div className="flex flex-col gap-4">
          {[...filteredData].map((entry, index) => {
            ...
            return (
              <div
                key={index}
                className="flex items-center gap-4 shadow-md rounded-2xl p-4 md:p-6"
              >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between w-full gap-1">
                  ...
                  <span
                    className={`text-xs font-semibold px-3 py-1 rounded-full w-fit ${config.color}`}
                  >
                    {config.label}
                  </span>
                </div>
